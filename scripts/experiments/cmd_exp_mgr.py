...
from c2hi.utils import try_to_declare_parameter
...
class HRIExpManager(Node):
    def __init__(self):
        ...
        # Non-ros parameters
        self.valid_cmd_modes = {'artag': ['artag'],
                                'gesture': ['gesture','gest_verb'],
                                'verbal': ['gest_verb','verbal']}
        self.package_dir = get_package_share_directory('c2hi')
        ...
        self.reconf_gesture_rec_client = self.create_client(Empty, 'gesture_rec_server/reconfigure')
    ...
    def run_experiments(self):
        ...
        for exp in self.exp_configs:
            ...
            while (('semantic_fusion_node' not in self.get_node_names()) or ('pra_node' not in self.get_node_names()) or ('speech_rec_node' not in self.get_node_names()) or ('gesture_rec_server' not in self.get_node_names())):
            ...
            subprocess.run(["ros2", "param", "load", "/gesture_rec_server", os.path.join(self.package_dir,exp_path)])
            ...
            rclpy.spin_until_future_complete(self, self.future)
            # while self.future.done() is False:
            #     self.get_logger().info("Could not reconfigure speech recognition node, retrying")
            #     self.future = self.reconf_speech_rec_client.call_async(self.empty_req)
            #     rclpy.spin_until_future_complete(self, self.future,timeout_sec=1)

            self.future = self.reconf_gesture_rec_client.call_async(self.empty_req)
            rclpy.spin_until_future_complete(self, self.future)
            ...
            self.get_logger().info("Command rec methods: %s" % cmd_rec_method)
            self.get_logger().info("Valid command modes: %s" % self.valid_cmd_modes[cmd_rec_method])
            ...
            for root, _, files in os.walk(self.mcap_dir):
                ...
                for file in files:
                    ...
                    if os.path.splitext(file)[-1] == '.mcap':
                        ...
                        if cmd_mode_actual not in self.valid_cmd_modes[cmd_rec_method]:
                            self.get_logger().info(f"Mode {cmd_mode_actual} cannot be recognized by {cmd_rec_method}. Skipping file...")
                            continue
