...
class SceneExpManager(Node):
    def __init__(self):
        ...
        self.reset_bayes_joint_scene_client = self.create_client(Empty, 'bayes_joint_scene_est/reset')
        self.reconf_bayes_clip_scene_client = self.create_client(Empty, 'bayes_clip_scene_est/reconfigure')
        self.reconf_bayes_joint_scene_client = self.create_client(Empty, 'bayes_joint_scene_est/reconfigure')
    ...
    def run_experiments(self):
        ...
        for exp in self.exp_configs:
            ...
            subprocess.run(["ros2", "param", "load", "/bayes_joint_scene_est", os.path.join(self.package_dir,exp_path)])
            ...
            self.future = self.reconf_bayes_joint_scene_client.call_async(self.empty_req)
            rclpy.spin_until_future_complete(self, self.future,timeout_sec=5)
            while self.future.done() is False:
                ...
                for file in files:
                    ...
                    if os.path.splitext(file)[-1] == '.mcap':
                        ...
                        if root != last_root: # Avoid playing mcap file multiple times
                            ...
                            bayes_joint_future = self.reset_bayes_joint_scene_client.call_async(Empty.Request())
                            rclpy.spin_until_future_complete(self, bayes_joint_future,timeout_sec=10)
